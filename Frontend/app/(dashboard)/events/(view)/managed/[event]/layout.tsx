...
export default async function EventLayout({params, children} : {children : ReactNode; params: { event: string }}){
  ...
  if (cookieStore) {
    ...
    if(arr[arr.length - 1].length==1 && (arr[arr.length - 1]=='0' || arr[arr.length - 1]=='1')){
        ...
        if(object?.["success"]){
          ...
          const checkEvent = objectData.filter((curr)=> (curr.eventId==eventId && curr.accType=='Manage'))
          if(checkEvent.length>0){
            console.log('Granted manager route priveleges to the user!')
            return (
